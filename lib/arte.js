...
class ArteClient {
    ...
    async getHomepage() {
        return cached('homepage', async () => {
            ...
            const data = await this._fetch(
                `${EMAC_BASE_URL}/${this.lang}/web/pages/HOME/`
            );
                ...
                for (const zone of data.value.zones) {
                    if (zone.content && zone.content.data) {
                        for (const item of zone.content.data) {
                            if (item.programId && item.stickers) {
                                // Vérifie que c'est un contenu jouable
                                const isPlayable = item.stickers.some(
                                    s => s.code === 'PLAYABLE' || s.code === 'FULL_VIDEO'
                                );
                                if (isPlayable) {
    ...
    /**
     * Récupère les vidéos d'une catégorie
     *
    ...
    async getCategory(category) {
        return cached(`category_${category}`, async () => {
            ...
            const data = await this._fetch(
                `${EMAC_BASE_URL}/${this.lang}/web/pages/${category}/`
            );
